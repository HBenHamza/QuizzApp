...
$.widget( "kf." + wizard, {
	...
	_create: function() {
		...
		self.elements.forward.click(function( event ) {
			event.preventDefault();
			var isValidStep = document.getElementsByClassName('ishidden-false');
			if (!isValidStep.length > 0) {
				self.forward( event );
			}
	...
	_update: function( event, state ) {
		var current = this._currentState,
			...
			var valid = true;
			$.each($(".step.current :input"),function(index,elt){
					if($('#'+elt.name).hasClass('required') && (elt.value == null || elt.value == "" || 
					$('.'+elt.name+'_emsg').hasClass('ishidden-false'))){
						valid = false;
						/*if(elt.value ==""){
							$('#'+elt.name).addClass('required');
						}*/
					}
			});
		...
		if ( current.step ) {
			if ( (o.disabled || !state ||
				...
				( !state.isMovingForward && !this._trigger( beforeBackward, event, state ) )) || !valid) {
...
$.extend($.validator, {
	...
	messages: {
		...
		maxlength: $.validator.format("La valeur maximale doit etre {0}."),
		minlength: $.validator.format("La valeur minimum doit etre {0}."),
		rangelength: $.validator.format("Please enter a value between {0} and {1} characters long."),
		range: $.validator.format("Please enter a value between {0} and {1}."),
		max: $.validator.format("La valeur maximale doit etre {0}."),
		min: $.validator.format("La valeur minimale doit etre {0}.")
	},
